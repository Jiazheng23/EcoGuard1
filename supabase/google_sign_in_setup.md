...
For Google registration, click Continue with Google from either authentication page. If the Google email has no EcoGuard profile, a modal asks the user to continue as Tourist or apply as Location Admin. An existing tourist email keeps its tourist role, and an existing location administrator keeps that role. Administrator access still requires the existing approval process. The Location Admin path needs the backend running with its existing admin credentials. If a database trigger automatically assigns every new Google account a tourist profile, remove that automatic role assignment before enabling this onboarding path; the application must make the role choice first.
...
When an unregistered user starts Google sign-in, EcoGuard returns to the sign-in page and displays a role-choice modal over the disabled, blurred authentication layout. Tourist opens the tourist dashboard. Location Admin opens the existing application process. Returning users skip this choice and open the dashboard associated with their existing role.
