...
async function run({ token = 'valid', google = true, role = 'tourist', concurrentRole = null, readError = null } = {}) {
  let storedRole = role
  let updates = 0
  const client = {
    auth: { getUser: async () => ({ data: { user: { id: 'user-1', identities: google ? [{ provider: 'google' }] : [] } } }) },
    from: () => ({
      select: () => ({ eq: (column, id) => {
        assert.equal(column, 'id')
        assert.equal(id, 'user-1')
        return { maybeSingle: async () => ({ data: storedRole ? { role: storedRole } : null, error: readError }) }
      } }),
      insert: async (values) => {
  ...
  await handler({ headers: token ? { authorization: `Bearer ${token}` } : {} }, response)
