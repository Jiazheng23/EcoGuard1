...
export function buildWasteCsv(collections = [], { locationName = 'EcoGuard location' } = {}) {
  ...
  const rows = collections.map((record) => [
    ...
    record.source === 'simulated_sensor' ? 'AUTOMATED SENSOR' : 'Manual entry',
    record.status,
    record.notes || '',
  ])
...
function buildSummaryPage({ locationName, filters, summary, trend, generatedAt }) {
  ...
  text(commands, 'SENSOR DATA SOURCE', 55, 254, 10, 'F2', [0.35, 0.18, 0.65])
  wrapText(commands, 'Rows labelled AUTOMATED SENSOR were captured from the location sensor feed and are reported separately from manual entries.', 55, 237, 475, 9, 13, [0.35, 0.18, 0.65])
  ...
  text(commands, 'Generated by the EcoGuard Waste Management System.', 42, 48, 8, 'F1', [0.5, 0.55, 0.62])
...
function buildTablePage(records, locationName, pageNumber) {
  ...
  records.forEach((record, index) => {
    ...
    const values = [
      ...
      fixedNumber(record.recycled_kg),
      fixedNumber(record.landfill_kg ?? Number(record.total_kg) - Number(record.recycled_kg)),
      record.source === 'simulated_sensor' ? 'SENSOR' : 'Manual',
  ...
  text(commands, 'Collection records are immutable. AUTOMATED SENSOR rows are identified separately from manual entries.', 36, 42, 7, 'F1', [0.45, 0.5, 0.56])
  return commands.join('\n')
}
