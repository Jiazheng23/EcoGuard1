...
export default function AdvisoryEditor({ source, advisory, locations = [], activeWarnings = [], isSuperAdmin = false, profile, onClose, onSaved }) {
  ...
  async function submit() {
    ...
    } catch (saveError) { const failure = saveError.message || 'Unable to publish advisory.'; setError(failure); toast.error(failure) }
  ...
  return createPortal(<div className="fixed inset-0 z-[9999] grid place-items-center bg-slate-950/65 p-4 backdrop-blur-[2px]" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose() }}><section className="flex max-h-[calc(100dvh-2rem)] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true"><header className="flex shrink-0 justify-between gap-4 border-b border-slate-100 px-6 py-5"><div><p className="text-xs font-bold uppercase tracking-widest text-blue-600">{advisory ? 'Update advisory' : 'Create tourist advisory'}</p><h2 className="mt-1 text-xl font-bold text-slate-900">{location?.name}</h2><p className="mt-1 text-sm text-slate-500">Source: {source?.description || advisory?.title || 'General location advisory'}</p></div><button type="button" onClick={onClose} disabled={saving} aria-label="Close advisory editor"><X className="text-slate-400" /></button></header><div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">{error && <p className="mb-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}
    <div className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-xs font-semibold text-slate-500">Advisory location<select value={locationId} disabled={!isSuperAdmin || sourceLocked || Boolean(advisory)} onChange={(event) => { setLocationId(event.target.value); setFieldErrors((current) => ({ ...current, locationId: undefined })) }} aria-invalid={Boolean(fieldErrors.locationId)} className={`mt-1 w-full rounded-xl border bg-white px-3 py-2.5 text-sm font-normal text-slate-700 disabled:bg-slate-100 ${fieldErrors.locationId ? 'border-red-400 focus:border-red-500 focus:ring-2 focus:ring-red-100' : 'border-slate-200'}`}><option value="">Select location</option>{locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{fieldErrors.locationId && <span className="mt-1 block font-normal text-red-500">{fieldErrors.locationId}</span>}<span className="mt-1 block text-[11px] font-normal text-slate-400">{sourceLocked ? 'Automatically routed from the selected source.' : isSuperAdmin ? 'Choose any managed destination.' : 'Locked to your assigned destination.'}</span></label>
      <Field label="Advisory title" value={form.title} onChange={(v) => change('title', v)} error={fieldErrors.title} />
      <Field label="Affected area within this destination" value={form.affected_area} onChange={(v) => change('affected_area', v)} error={fieldErrors.affected_area} />
      ...
      <Field label="Safety instructions" value={form.safety_instructions} onChange={(v) => change('safety_instructions', v)} area error={fieldErrors.safety_instructions} />
