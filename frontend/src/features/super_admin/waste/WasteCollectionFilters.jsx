...
export default function WasteCollectionFilters({ filters, onChange }) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
      ...
      <FilterField label="Source"><select name="source" value={filters.source} onChange={updateFilter} className={filterClass()}><option value="all">All sources</option>{WASTE_COLLECTION_SOURCES.map((source) => <option key={source} value={source}>{source === 'simulated_sensor' ? 'Automated sensor' : 'Manual'}</option>)}</select></FilterField>
