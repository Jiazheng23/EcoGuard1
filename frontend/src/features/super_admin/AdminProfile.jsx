...
export default function AdminProfile({ user, profile, onProfileChange }) {
  ...
  async function saveProfile(event) {
    ...
    try {
      ...
      const failure = saveError.message || 'Unable to update the administrator profile.'
      setError(failure)
      toast.error(failure)
    } finally {
      setSaving(false)
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
        <aside className={`${card} h-full text-center`}>
          <ProfileAvatarUploader userId={user.id} profile={profile} initials={initials} accent="blue" onProfileChange={onProfileChange} onError={(text) => { setError(text); toast.error(text) }} onSuccess={(text) => { setMessage(text); toast.success(text) }} />
        ...
        <form onSubmit={saveProfile} noValidate className={`${card} space-y-5`}>
          ...
          <Field label="Full Name" icon={UserRound} value={fullName} onChange={(value) => updateField('fullName', setFullName, value)} required maxLength={120} error={fieldErrors.fullName} />
          <Field label="Email Address" icon={Mail} value={user?.email || ''} readOnly />
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Phone Number" icon={Phone} value={phone} onChange={(value) => updateField('phone', setPhone, value)} placeholder="Example: +60 12-345 6789" type="tel" inputMode="tel" error={fieldErrors.phone} />
...
function Field({ label, icon: Icon, value, onChange, readOnly, required, placeholder, type = 'text', inputMode, error, maxLength }) {
  return <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-500">{label}</span><span className="relative block"><Icon className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input type={type} inputMode={inputMode} value={value} onChange={onChange ? (event) => onChange(event.target.value) : undefined} placeholder={placeholder} readOnly={readOnly} required={required} maxLength={maxLength} aria-invalid={Boolean(error)} className={`w-full rounded-xl border py-2.5 pl-10 pr-3 text-sm outline-none ${readOnly ? 'cursor-not-allowed border-slate-100 bg-slate-100 text-slate-500' : error ? 'border-red-400 bg-red-50 text-slate-700 focus:border-red-500 focus:ring-2 focus:ring-red-100' : 'border-slate-200 bg-slate-50 text-slate-700 focus:border-blue-500'}`} /></span>{error && <span className="mt-1 block text-xs font-normal text-red-500">{error}</span>}</label>
}
