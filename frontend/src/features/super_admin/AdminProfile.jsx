...
export default function AdminProfile({ user, profile, onProfileChange }) {
  ...
  async function saveProfile(event) {
    ...
    setSaving(true)
    try {
      const updated = await updateOwnProfile(user.id, { fullName, phone, avatarUrl: profile?.avatar_url })
      const { error: authError } = await supabase.auth.updateUser({ data: { ...user.user_metadata, full_name: updated.full_name, phone: updated.phone } })
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
        <aside className={`${card} h-fit text-center`}>
          ...
          <span className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700"><ShieldCheck size={13} />Location Administrator</span>
          <div className="mt-5 border-t border-slate-100 pt-4 text-left"><p className="text-xs font-medium text-slate-400">Member since</p><p className="mt-1 text-sm text-slate-600">{joined}</p></div>
        </aside>
        ...
        <form onSubmit={saveProfile} className={`${card} space-y-5`}>
          ...
          <div className="grid gap-4 sm:grid-cols-2"><Info label="Account Role" value={profile?.role === 'admin' ? 'Location Admin' : profile?.role || 'Unknown'} icon={ShieldCheck} /><Info label="Member Since" value={joined} icon={CalendarDays} /></div>
