...
const features = [
  ...
  { icon: BarChart3, title: 'Carbon Analytics', description: 'Detailed trip-by-trip emission breakdowns' },
  { icon: ShieldCheck, title: 'Early Warnings', description: 'Alerts before ecological damage occurs' },
]
  ...
  return (
    <main className="auth-page">
      <aside className="auth-showcase">
        <div>
          <Link className="auth-back auth-back--light" to="/login"><ArrowLeft size={16} /> Back to Home</Link>
          <div className="auth-brand"><span className="auth-brand__mark"><ShieldCheck size={21} /></span><span>EcoGuard EEWS</span></div>
          <h2>Monitor.<br />Protect.<br />Sustain.</h2>
          <p>Join Malaysia's ecological early warning system. Track your carbon footprint, monitor environmental conditions, and contribute to sustainable tourism.</p>
        </div>
        <div className="auth-features">
          {features.map(({ icon: Icon, title, description }) => (
            <div className="auth-feature" key={title}><span><Icon size={17} /></span><div><strong>{title}</strong><small>{description}</small></div></div>
          ))}
        </div>
      </aside>

      <section className="auth-content">
        <Link className="auth-back auth-back--mobile" to="/login"><ArrowLeft size={16} /> Back</Link>
        <div className="auth-card">
          <header><h1>{heading[0]}</h1><p>{heading[1]}</p></header>

          {mode !== 'forgot' && <div className="role-picker">
            <button className={role === 'tourist' ? 'selected' : ''} onClick={() => setRole('tourist')} type="button"><span><UserRound size={20} /></span><strong>Tourist</strong><small>Track your eco impact</small></button>
            <button className={role === 'admin' ? 'selected' : ''} onClick={() => setRole('admin')} type="button"><span><ShieldCheck size={20} /></span><strong>Location Admin</strong><small>Manage your location</small></button>
          </div>}
          ...
          <p className="auth-terms">Demo mode: any email and password can access the tourist dashboard.</p>
        </div>
      </section>
    </main>
  )
...
function Field({ label, icon, name, type = 'text', placeholder, value, onChange }) {
  return <div className="field"><label htmlFor={name}>{label}</label><div className="field__input">{icon}<input id={name} name={name} type={type} placeholder={placeholder} value={value} onChange={onChange} required /></div></div>
}
